...
key_bits=2048
expire_days=3650
subj1=/C="CN"/ST="Beijing"/L="RSA1"/O="Fortinet"/OU="ecdsa"/CN="ecdsa-intermediate1"
subj2=/C="CN"/ST="Beijing"/L="RSA1"/O="Fortinet"/OU="ecdsa"/CN="ecdsa-intermediate2"
subj=/C="CN"/ST="Beijing"/L="RSA1"/O="Fortinet"/OU="ecdsa"/CN="domain3.net"
ca_name=ca-root
root_cacer=$ca_name.cer
root_cakey=$ca_name.key
param=ecdsa-self-sign
...
openssl ca -extensions v3_ca -batch -notext -in $csr -out $sub1_cacer -selfsign
echo "===================Gen Sub1 CA OK===================="

#Sub2 CA
openssl genrsa -out $cakey $key_bits
openssl req -new -key $cakey -sha256 -out $csr -subj $subj2 -days $expire_days
openssl ca -extensions v3_ca -batch -notext -in $csr -out $cacer -cert $sub1_cacer -keyfile $sub1_cakey -selfsign
